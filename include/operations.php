<?php 
    ...
    function create_forum_post($username, $title, $image_url, $category, $content) {
        ...
        $result = mysqli_query($conn, $sql);
        if($result) {
            //TODO transfer to post page when new post is made
    ...
    function generate_post_html($post_id, $username, $title, $category, $content, $image_url, $post_date, $is_post_head) {
        ...
        $str .= '<div class="forum-main"><div class="title-wrapper"><h4>' . $title . '</h4></div>';
    ...
    function generate_comment_html($comment_id, $username, $title, $category, $content, $image_url, $post_date) {
